...
const SignupPage = () => {
  ...
  const [isGoogleFlowPending, setIsGoogleFlowPending] = useState(false); // NEW: true while GoogleAuthButton is showing its "choose profile type" card
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-8 py-10 sm:py-14">
        <div className="w-full max-w-lg">
          ...
          <GoogleAuthButton
            ...
            disabled={isLoading}
            text="signup_with"
            onPendingChange={setIsGoogleFlowPending} // NEW: hide email signup section while Google's profile-type card is open
          />

          {/* NEW: entire "or sign up with email" section is hidden while Google's own profile-type picker is showing, so the user only sees one "register as" control at a time */}
          {!isGoogleFlowPending && (
            <>
