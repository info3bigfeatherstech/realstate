// src/tabs/SubscriptionsTab/SubscriptionsTab.jsx
import React, { Suspense } from "react";
import { useSearchParams } from "react-router-dom";
import { SUBSCRIPTIONS_REGISTRY } from "./SubscriptionRegistry";

const SubscriptionsTab = () => {
  const [searchParams, setSearchParams] = useSearchParams();

  const activeSubTab = searchParams.get("ctab") || "plans";
  ...
  const activeConfig = SUBSCRIPTIONS_REGISTRY.find((sub) => sub.id === activeSubTab) || SUBSCRIPTIONS_REGISTRY[0];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200 overflow-x-auto whitespace-nowrap">
        {SUBSCRIPTIONS_REGISTRY.map((sub) => {
