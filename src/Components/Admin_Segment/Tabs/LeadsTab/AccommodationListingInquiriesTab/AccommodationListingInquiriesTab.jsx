...
} from "../../../Admin_Redux/AccommodationListingInquiryApi/accommodationListingInquiryApi";
import {
    setPage,
    setLimit,
    setSearch,
    ...
    setCity,
    resetFilters,
} from "../../../Admin_Redux/AccommodationListingInquiryApi/accommodationListingInquirySlice";
...
const AccommodationListingInquiriesTab = () => {
    const dispatch = useDispatch();
    const filters = useSelector((state) => state.accommodationListingInquiry);
    const { data: constants } = useGetConstantsQuery();
    const { data: stats, isLoading: statsLoading } = useGetAccommodationListingInquiryStatsQuery();
    const { data, isLoading, isFetching } = useGetAccommodationListingInquiriesQuery(filters);
    ...
    const handleDownloadTemplate = async () => {
        ...
        try {
            await downloadAccommodationListingTemplate();
            toast.success("Accommodation Listing form template downloaded");
        } catch (err) {
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h3 className="text-lg font-semibold text-slate-800">Accommodation Listing Inquiries</h3>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                    ...
                    >
                        ...
                        {(c.LISTING_PROPERTY_TYPES || []).map((t) => (
                            <option key={t} value={t}>{t}</option>
                        ))}
                    </select>
                    ...
                    <select
                        className="h-9 px-2 rounded-lg border border-slate-300 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-blue-100"
                        value={filters.listingUrgency || ""}
                        onChange={(e) => dispatch(setListingUrgency(e.target.value))}
                    >
                        <option value="">All Urgency</option>
                        {(c.LISTING_URGENCY_OPTIONS || []).map((u) => (
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                {isLoading ? (
                    ...
                    <p className="text-center text-slate-400 py-16 text-sm">No accommodation listing inquiries found</p>
                ) : (
                            ...
                            <tbody>
                                {inquiries.map((inq) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-slate-600">Accommodation Listing</td>
                                        ...
                                        <td className="px-4 py-3">
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setSelectedId(inq._id); }}
...
export default AccommodationListingInquiriesTab;