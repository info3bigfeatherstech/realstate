    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Define loyalty badge levels, deal thresholds, color schemes, and
            rewards for agents.
          </p>
        </div>
        <button
        ...
        >
          <Plus className="w-4 h-4" /> Add Badge Tier
        </button>
      </div>
      ...
      {sortedTiers.length === 0 ? (
        <div className="text-center py-16 border-2 border-dashed border-slate-200 rounded-2xl">
          <p className="text-sm text-slate-400">No badge tiers defined yet.</p>
          <button
            onClick={handleAddClick}
              ...
              <tbody className="divide-y divide-slate-100">
                {sortedTiers.map((tier) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center text-slate-600 font-mono">
                      {tier.maxDeals ?? "Unlimited"}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span
                          className="w-5 h-5 rounded-full border border-slate-200 block shrink-0"
